"""Seed-level inference for exact DREAM checkpoint head-to-head values."""
...
def exact_one_sided_sign_flip_p(values: Sequence[float]) -> float:
    """Exact randomisation p-value for a positive mean under sign symmetry."""
...
def _row_checkpoint(row: Mapping[str, object], lower: str, upper: str) -> int:
    return int(row[lower] if lower in row else row[upper])
...
def build_inference_tables(
    ...
    """Build seed effects, headline summaries, and pairwise inference rows."""
    ...
    for row in pairwise_rows:
        seed = str(row["seed"])
        checkpoint_a = _row_checkpoint(row, "checkpoint_a", "checkpoint_A")
        checkpoint_b = _row_checkpoint(row, "checkpoint_b", "checkpoint_B")
    ...
    primary_values = [row["mean_later_vs_earlier_ev"] for row in seed_rows]
    adjacent_values = [row["mean_adjacent_checkpoint_ev"] for row in seed_rows]
    final_first_values = [row["final_vs_first_ev"] for row in seed_rows]
    summary_rows = [
        {
        ...
        },
        {
            ...
            **_summary(adjacent_values),
        },
        {
            "estimand": "final_checkpoint_ev_vs_first_checkpoint",
            **_summary(final_first_values),
        },
    ]
    ...
    adjusted = _holm_adjust([row["exact_one_sided_sign_flip_p"] for row in pair_rows])
