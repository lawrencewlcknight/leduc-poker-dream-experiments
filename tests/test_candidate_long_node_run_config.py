"""Configuration checks for Experiment 41 candidate long-node run."""
...
from experiments.leduc_poker.dream_candidate_long_node_run.config import (
...
def test_candidate_long_node_run_is_single_five_seed_arm():
    ...
    assert EXPERIMENT_CONFIG["epsilon"] == LONG_NODE_EPSILON == 0.06
    ...
    assert EXPERIMENT_CONFIG["baseline_variant"] == CANDIDATE_LONG_NODE_VARIANT
    assert EXPERIMENT_CONFIG["treatment_keys"] == ["epsilon", "num_traversals"]
    assert [variant["variant_id"] for variant in CANDIDATE_LONG_NODE_VARIANTS] == [
        CANDIDATE_LONG_NODE_VARIANT,
    ]


def test_candidate_long_node_run_preserves_experiment_22_candidate_spec():
    config = apply_variant_overrides(EXPERIMENT_CONFIG, CANDIDATE_LONG_NODE_VARIANTS[0])
...
def test_candidate_long_node_run_cli_does_not_enable_fixed_baseline_by_default():
