#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for DREAM Experiment 44."""
...
if gcloud storage ls "$REMOTE_TASK/**" >/dev/null 2>&1; then
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration, cpu, memory, machine, disk = "64800s", 8000, 30000, "n2-standard-8", 150
    elif args.kind == "smoke":
        max_duration, cpu, memory, machine, disk = "7200s", 8000, 30000, "n2-standard-8", 100
    elif args.kind == "controller":
        max_duration, cpu, memory, machine, disk = "259200s", 1000, 1500, "e2-small", 30
    else:
        max_duration, cpu, memory, machine, disk = "14400s", 4000, 15000, "n2-standard-4", 50
    return {
        "taskGroups": [
            {
                "taskSpec": {
                    ...
                    "maxRetryCount": 1 if args.kind == "controller" else 0,
