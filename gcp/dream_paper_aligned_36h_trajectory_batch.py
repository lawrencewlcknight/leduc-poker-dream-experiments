#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for DREAM Experiment 46."""
...
MODULE = "experiments.leduc_poker.dream_paper_aligned_36h_trajectory.cloud"
TASK_COUNT = 5
...
WORK_ROOT="/workspace/drm46-{kind}-$TASK_SLOT-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
VENV_ROOT="/tmp/drm46-{kind}-venv-$TASK_SLOT-$RETRY_ATTEMPT"

if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
$SUDO apt-get install -y git curl ca-certificates python3 python3-venv python3-dev build-essential
mkdir -p "$WORK_ROOT" "$MPLCONFIGDIR" "$XDG_CACHE_HOME" "$UV_CACHE_DIR" \
...
TASK_METADATA="$(python - "$TASK_INDEX" <<'PY' | sed -n 's/^DRM46_TASK_METADATA //p' | tail -n 1
import importlib
import sys
module = importlib.import_module("{MODULE}")
index = int(sys.argv[1])
seeds = tuple(module.PRODUCTION_SEEDS)
print("DRM46_TASK_METADATA", int(seeds[index]), module.task_name(index, seeds))
PY
)"
...
if [[ ! "$SOURCE_SEED" =~ ^[0-9]+$ \
  ...
  echo "Invalid Experiment 46 task metadata: $TASK_METADATA" >&2
  exit 2
fi
""".strip()
...
WORK_ROOT="/workspace/drm46-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export DRM46_REMOTE_CONTROLLER=1

gcloud batch jobs list --project "$PROJECT_ID" --location "$REGION" --limit 1 \
  --format='value(name)' >/dev/null
exec bash gcp/run_dream_paper_aligned_36h_trajectory.sh "$CONTROLLER_ACTION"
"""
...
gcloud storage rsync --recursive --exclude='(^|/)raw_endpoint_reservoirs(/|$)' \
  "$REMOTE_TRAINING" "$OUTPUT_ROOT/training/$TASK_NAME"
...
gcloud storage rsync --recursive "$BUCKET_ROOT/$RUN_ID/evaluation" "$OUTPUT_ROOT/evaluation"
python -m {MODULE} aggregate --evaluation-root "$OUTPUT_ROOT" \
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        duration, cpu, memory, machine, disk, retries = (
            "172800s", 8000, 30000, "n2-standard-8", 200, 0
        )
    elif args.kind == "evaluate":
        duration, cpu, memory, machine, disk, retries = (
            "72000s", 8000, 30000, "n2-standard-8", 200, 0
        )
    elif args.kind == "smoke":
        duration, cpu, memory, machine, disk, retries = (
            "7200s", 8000, 30000, "n2-standard-8", 100, 0
        )
    elif args.kind == "controller":
        duration, cpu, memory, machine, disk, retries = (
            "604800s", 1000, 1500, "e2-small", 30, 2
        )
    else:
        duration, cpu, memory, machine, disk, retries = (
            "7200s", 4000, 15000, "n2-standard-4", 100, 0
        )
    return {
        "taskGroups": [
            {
        ...
        "labels": {"experiment": "dream-paper-36h", "stage": args.kind},
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--kind", choices=("controller", "smoke", "train", "evaluate", "aggregate"), required=True
    )
    ...
    parser.add_argument(
        "--controller-action", choices=("orchestrate", "orchestrate-resume"), default="orchestrate"
    )
