...
BUILDER="$SCRIPT_DIR/dream_paper_aligned_36h_trajectory_batch.py"
MODULE="experiments.leduc_poker.dream_paper_aligned_36h_trajectory.cloud"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/drm46-paper36h-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/drm46-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/drm46-cache}"
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 46 commit SHA}"
...
RUN_ID="${RUN_ID:-drm46-paper36h-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d "/tmp/drm46-batch.XXXXXX")"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/drm46_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/drm46_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/drm46_train_job.json"
    cp "$TEMP_DIR/evaluate.json" "$REPO_DIR/drm46_evaluate_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/drm46_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote DREAM Experiment 46 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${DRM46_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
