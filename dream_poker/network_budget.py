"""Reusable helpers for DREAM network-training budget ablations."""
...
def add_budget_curve_columns(curves: pd.DataFrame, config: Dict, budget_key: str) -> pd.DataFrame:
    ...
    curves["baseline_loss_mean"] = curves[["baseline_loss_player_0", "baseline_loss_player_1"]].mean(axis=1)
    curves["baseline_grad_norm_mean"] = curves[
        ["baseline_grad_norm_player_0", "baseline_grad_norm_player_1"]
    ].mean(axis=1)
    return curves


def summarise_network_budget_curve(
    curves: pd.DataFrame,
    seed: int,
    variant: Dict,
    config: Dict,
    ...
    return {
        ...
        budget_key: int(config[budget_key]),
        ...
        "final_baseline_loss_mean": float(final_row.get("baseline_loss_mean", np.nan)),
        "final_baseline_grad_norm_mean": float(final_row.get("baseline_grad_norm_mean", np.nan)),
...
def aggregate_network_budget_summary(
    ...
    for variant, group in summary_df.groupby("variant", sort=False):
        row = {
            ...
            budget_key: int(group[budget_key].iloc[0]),
        }
...
def create_network_budget_plots(
    ...
    plot_curve_by_variant(
        ...
        "iteration",
        "baseline_loss_mean",
        f"{title_prefix}: Baseline Loss Diagnostic",
        "Mean baseline loss",
        variant_order,
        variant_labels,
    ...
    plot_curve_by_variant(
        ...
        f"{title_prefix}: Baseline-Replay Reward Variance",
    ...
    if len(paired_df):
        ...
        plot_paired_delta_bar(
            ...
            f"{title_prefix}: Paired Advantage-Variance Difference",
            "Variant - baseline",
            variant_order,
            variant_labels,
